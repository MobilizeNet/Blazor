namespace SKS_Blazor.Components
{
    ...
    public partial class frmAdjustStockManual: Form
	{

		public frmAdjustStockManual(): base()
		{
        ...
        private async void cmdSave_Click(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
                ...
                if (await MessageBox.Show($"Data modified successfully{Environment.NewLine}Would you like to modify another stock manually?", "Modify data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
				{
					ClearFields();
				}
				else
			...
			catch (System.Exception excep)
			{
                ...
                // gap-note: Await the modal result.
				await MessageBox.Show($"An error has occurred adding the data. Error: ({Information.Err().Number.ToString()}) {excep.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

        }

        // gap-note: Change this method to async so it can wait for the modal result.
        private async void Form_FormClosing(Object eventSender, FormClosingEventArgs eventArgs)
		{
			...
			try
			{
				...
				{
                    // gap-note: Await the modal result.
                    res = await MessageBox.Show("Do you want to save the edited data?", "Save data", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
		...
		private async void cmdProducts_Click(Object eventSender, EventArgs eventArgs)
		{			
			...
			else
			{
				while (!modConnection.rs.EOF)
				{
					...
					for (modMain.i = 1; modMain.i <= tempForEndVar; modMain.i++)
					{
						...
						{
                            ListView.GetListViewSubItem(x, modMain.i).Text = Convert.ToString(modConnection.rs[modMain.i]);
						}
					}
        ...
        // gap-note: Change this method to async so it can wait for the modal result.
        private async void DoSearchStocks()
		{
			...
			{
                // gap-note: Await the modal result.
                if (await MessageBox.Show("Do you want to cancel previous edited data?", "Data edition", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
				{
					return;
				}
			}
			string productId = lvProducts.FocusedItem.Text;
            string productName = ListView.GetListViewSubItem(lvProducts.FocusedItem, 1).Text;
			currentIdProduct = lvProducts.FocusedItem.Text;
			ListViewItem withVar = null;
			withVar = lvProducts.FocusedItem;
			//UPGRADE_WARNING: (2080) IsEmpty was upgraded to a comparison and has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis/warnings#id-2080
			if (!String.IsNullOrEmpty(ListView.GetListViewSubItem(withVar, 5).Text))
			{
				currentQuantityPerUnit = ListView.GetListViewSubItem(withVar, 5).Text;
			}
			//UPGRADE_WARNING: (2080) IsEmpty was upgraded to a comparison and has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis/warnings#id-2080
			if (!String.IsNullOrEmpty(ListView.GetListViewSubItem(withVar, 6).Text))
			{
				currentUnit = ListView.GetListViewSubItem(withVar, 6).Text;
			}
			currentProductName = ListView.GetListViewSubItem(withVar, 1).Text;
        ...
        // gap-note: Change this method to async so it can wait for the modal result.
        private async void RetrieveDataProduct()
		{
			if (editingData)
			{
                // gap-note: Await the modal result.
			...
			if (lvStocks.FocusedItem != null)
			{
				...
				if (!(lvStocks.FocusedItem is null))
				{
                    ...
                    currentStock = Double.Parse(ListView.GetListViewSubItem(withVar, 1).Text);
					currentStockPrice = Double.Parse(ListView.GetListViewSubItem(withVar, 4).Text);
