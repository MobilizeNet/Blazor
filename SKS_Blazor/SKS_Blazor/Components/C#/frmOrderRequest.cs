namespace SKS_Blazor.Components
{
    ...
    public partial class frmOrderRequest: Form
    {
        ...
        private async void cmdCustomers_Click(Object eventSender, EventArgs eventArgs)
        {
        ...
        private async void cmdSave_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            try
            {
                ...
                editingData = false;
                if (await MessageBox.Show($"Order added successfully{Environment.NewLine}Would you like to add a new order?", "New data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == Gap.Blazor.DialogResult.Yes)
                {
                    ClearFields();
                }
                else
        ...
        private async void RetrieveDataCustomer()
        {
            if (editingData)
            {
                if (await MessageBox.Show("Do you want to cancel previous edited data?", "Data edition", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != Gap.Blazor.DialogResult.Yes)
                {
                    return;
                }
            }
            ...
            if (!(lvCustomers.FocusedItem is null))
            {
                ...
                currentCompanyName = ListView.GetListViewSubItem(withVar, 1).Text;
