...
namespace SKS_Blazor.Components
{
		...
		: Form
	{
		...
		private async void cmdApprove_Click(Object eventSender, EventArgs eventArgs)
		{
			try
			{
				...
				LoadData();
				await MessageBox.Show("The order was successfully approved", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()));
				this.Close();
			}
		...
		private async void cmdCancel_Click(Object eventSender, EventArgs eventArgs)
		{
			try
			{
				if (txtStatus.Text.ToUpper() == "CANCELLED")
				...
				if (await MessageBox.Show("Do you want to cancel the order reception?", "Confirm cancellation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
				{
					return;
				}
				...
				LoadData();
				await MessageBox.Show("The order was successfully cancelled", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()));
				this.Close();
			}
			catch (System.Exception excep)
			{
