...
namespace SKS_Blazor.Components
{
		...
		: Form
	{
		...
		private void frmAddProductTo_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
			...
			{
                // gap-note: This should be generated as a stub.
		...
		private async void cmdSave_Click(Object eventSender, EventArgs eventArgs)
		{
			...
			{
				editingData = true;
				await MessageBox.Show("No data to be saved", "No data modified", MessageBoxButtons.OK, MessageBoxIcon.Information);
				this.Close();
				return;
			}
			SavedChanges = true;
			...
			editingData = false;
            await MessageBox.Show("Data was succesfully saved", "New data", MessageBoxButtons.OK, MessageBoxIcon.Information);
			this.Close();
			return;

            //UPGRADE_WARNING: (2081) Err.Number has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis/warnings#id-2081
            await MessageBox.Show($"An error has occurred adding the data. Error: ({Information.Err().Number.ToString()}) {Information.Err().Description}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
		}
		...
		private void LoadProductsById()
		{
			...
			if (modConnection.rs.RecordCount > 0)
			{
				while (!modConnection.rs.EOF)
				{
					...
					ListView.GetListViewSubItem(x, 3).Text = $"{Convert.ToString(modConnection.rs[3])}{Convert.ToString(modConnection.rs[4])}";
		...
		private void AddProductToSet()
		{
			...
			if (!(lvProducts.FocusedItem is null))
			{
				...
				if (!found)
				{
					...
					ListView.GetListViewSubItem(x, 1).Text = ListView.GetListViewSubItem(y, 1).Text;
					ListView.GetListViewSubItem(x, 2).Text = ListView.GetListViewSubItem(y, 2).Text;
					ListView.GetListViewSubItem(x, 3).Text = $"{ListView.GetListViewSubItem(y, 5).Text}{ListView.GetListViewSubItem(y, 6).Text}";
				}
			}
		}
