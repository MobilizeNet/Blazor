namespace SKS_Blazor.Components
{
	...
	public partial class frmOrderReception:  Form
	{

		public frmOrderReception(): base()
		{
		...
		private async void cmdAddProducts_Click(Object eventSender, EventArgs eventArgs)
		{
		...
		private async void cmdProviders_Click(Object eventSender, EventArgs eventArgs)
		{
		...
		private async void cmdSave_Click(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
				...
				if (await MessageBox.Show($"Order reception added successfully{Environment.NewLine}Would you like to add a new order reception?", "New data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) ==  DialogResult.Yes)
				{
					ClearFields();
				}
				else
		...
		private void MakeTextBoxVisible(TextBox txtBox, DataGridViewFlex grid)
		{
		...
		private void EditKeyCode(DataGridViewFlex grid, ref TextBox txtBox, int KeyCode, int Shift)
		{
			switch(KeyCode)
			{
				case 27 :  //ESC 
		...
		// gap-note: Change this method to async so it can wait for the modal result.
		private async void RetrieveDataProvider()
		{
			if (editingData)
			{
				// gap-note: Await the modal result.
			...
			if (!(lvProviders.FocusedItem is null))
			{
				...
				currentProviderName = ListView.GetListViewSubItem(withVar, 1).Text;
				currentContactName = $"{ListView.GetListViewSubItem(withVar, 2).Text} {ListView.GetListViewSubItem(withVar, 3).Text}";
